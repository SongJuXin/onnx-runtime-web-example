...
async function inferenceSqueezenet(path) {
  ...
  // 2. Run model
  const [predictions, inferenceTime] = await runSqueezenetModel(imageTensor,['webgl']);
  const [wasmPredictions, wasmInferenceTime] = await runSqueezenetModel(imageTensor,['wasm']);
  // 3. Return predictions and the amount of time it took to inference.
  return [
    [predictions, inferenceTime],
    [wasmPredictions,wasmInferenceTime]
  ];
}
...
async function runSqueezenetModel(preprocessedData,executionProviders=['webgl']){
  // Create session and set options. See the docs here for more options:
  //https://onnxruntime.ai/docs/api/js/interfaces/InferenceSession.SessionOptions.html#graphOptimizationLevel
  const session = await ort.InferenceSession
    .create('mobilenet/mobilenetv2-7.onnx',
